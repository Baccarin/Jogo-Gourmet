...
public class Tela extends JFrame {
	...
	public void beginScreen(JFrame frame) {
		int resp = JOptionPane.showConfirmDialog(frame, "Pense em um prato que você gosta", this.titulo,
	...
	public int question(JFrame frame, Prato prato) {
		...
		return JOptionPane.showConfirmDialog(frame,
				String.format("O prato que você pensou foi %s", prato.getClassificacao().getClassificacao()),
				this.titulo, JOptionPane.YES_NO_OPTION);
	}

	public int nameQuestion(JFrame frame, Prato prato) {
		return JOptionPane.showConfirmDialog(frame, String.format("O prato que você pensou foi %s", prato.getNome()),
				this.titulo, JOptionPane.YES_NO_OPTION);
	}
	...
	public void addNewPrato(String prato, JFrame frame) {
		...
		String newClassificacao = JOptionPane
				.showInputDialog(String.format("%s é ____ mas %s não", newPrato.getNome(), pratos.get(0).getNome()));
	...
	public void printPrato(Prato p) {
		System.out.println(String.format("O prato adicionado foi: [%s] que é um(a) [%s]", p.getNome(), p.getClassificacao().getClassificacao()));
	}

	public void janelaPrato(JFrame frame, Prato p) {
		JOptionPane.showConfirmDialog(frame,
				(String.format("O prato adicionado foi: [%s] que é um(a) [%s]", p.getNome(), p.getClassificacao().getClassificacao())),
